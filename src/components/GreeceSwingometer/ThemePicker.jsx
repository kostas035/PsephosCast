  ...
  return (
    ...
    >
      ...
      <div style={{ padding: "9px 11px", display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 8 }}>
        <div>
          <div style={{ fontSize: 12, fontWeight: 700, color: "var(--text-title)", fontFamily: "var(--ff-body)", letterSpacing: 0.3 }}>{theme.label}</div>
...
export default function ThemePicker({ current, onSelect, currentPalette, onSelectPalette, onClose, isMobile }) {
    ...
    >
      ...
      >
        ...
        <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "1fr 1fr 1fr", gap: 10 }}>
          {THEME_ORDER.map(id => (
            <ThemeCard key={id} theme={THEMES[id]} active={current === id} onSelect={onSelect} />
          ))}
        </div>
        ...
        <div style={{ marginTop: 10, padding: "10px 12px", background: "var(--btn-bg)", border: "1px solid var(--border)", borderRadius: 8 }}>
          <div style={{ fontSize: 11, color: "var(--text-muted)", fontFamily: "var(--ff-body)", marginBottom: 8, lineHeight: 1.4 }}>{pal.desc}</div>
