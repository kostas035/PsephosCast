...
export const GrMetricsCards = memo(function GrMetricsCards({ electionResult, parties, isMobile, turnout, isEstimate }) {
  ...
  const cards = [
    { 
      label: "First Place",    
      value: winner ? `${winner.nationalPct?.toFixed(1)}%` : "None", 
      sub: wp?.fullName || winner?.name || "No qualifying party",                         
      accent: wp?.color || "var(--text-muted)" 
    },
    { 
      label: "Winner Seats",   
      value: winner ? winner.seats : 0,                         
      sub: bonusSeats > 0 ? `incl. +${bonusSeats} bonus` : "no bonus seats", 
      accent: wp?.color || "var(--text-muted)" 
    },
    { 
      label: "Turnout (est.)", 
      value: grFmtVotesShort(turnout),              
      sub: isEstimate ? "projected estimate" : "actual votes cast",             
      accent: "#60A5FA" 
    },
    { 
      label: hasMaj ? "Majority" : "Seats Short",
      value: winner ? (hasMaj ? `+${winner.seats - GR.MAJORITY}` : `${GR.MAJORITY - winner.seats}`) : GR.MAJORITY,
      sub: hasMaj ? `${winner.seats} of ${GR.MAJORITY} needed` : "coalition required",
...
export default memo(function ResultsTable({ electionResult, parties, turnout, isEstimate, threshold, scenarioId }) {
  ...
  const deltaLabel = scenarioId === "2019" ? "Seats (vs '19)" : "Seats (vs '23)";

  return (
    <div style={S.card}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
        <div style={S.label}>Seat Distribution</div>
        {turnout > 0 && <div style={{ fontSize: 9, color: "var(--text-dim)", ...S.mono, letterSpacing: 1 }}>{isEstimate ? "EST. " : ""}{grFmtVotesShort(turnout)} total votes</div>}
      </div>
      <div style={{ overflowX: "auto" }}>
        <table style={{ width: "100%", borderCollapse: "collapse", minWidth: turnout ? 500 : 440 }}>
          <thead><tr>
            {["Party", "Vote %", "After Threshold", turnout ? "Votes" : null, deltaLabel, "Bonus", "Share"].filter(Boolean).map(h => (
      ...
      {bonusSeats > 0 && (
        <div style={{ marginTop: 12, padding: "8px 12px", background: "var(--bonus-bg)", borderRadius: 4, border: "1px solid var(--bonus-border)", display: "flex", alignItems: "center", gap: 8 }}>
          ...
          <div style={{ fontSize: 9, color: "#92400E", fontFamily: "var(--ff-body)", letterSpacing: 0.5, lineHeight: 1.5 }}>
            <strong style={{ color: "#D97706" }}>{bonusSeats} bonus seats</strong> — Winner at {winnerPct?.toFixed(1)}% ({GR.BONUS_BASE} base +{bonusSeats - GR.BONUS_BASE} extra) | Proportional pool: {proportionalPool} seats
          </div>
        </div>
      )}
            ...
            <div style={{ display: "flex", flexWrap: "wrap", gap: "3px 12px", marginTop: 4 }}>
              {eliminatedDetail.map(p => (
                <div key={p.id} style={{ display: "flex", alignItems: "center", gap: 4, fontSize: 9, ...S.mono, color: "var(--text-muted)" }}>
                  <span style={{ color: GR_PARTY_DICT[p.id]?.color || "var(--text-muted)" }}>{p.name}</span>
