      ...
      <div style={{ padding: 32, width: "100%", maxWidth: 720, maxHeight: "85vh", overflowY: "auto", background: "var(--bg-base)", borderRadius: 12, border: "1px solid var(--border)", boxShadow: "0 25px 50px -12px rgba(0, 0, 0, 0.5)", color: "var(--text-main)", fontFamily: "var(--ff-body)" }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 24, fontSize: 14, lineHeight: 1.65 }}>
          ...
          <Section step="C1" title={t("Demographic swing model")}>
            <p style={{ margin: 0 }}>
              <T lang={lang}
                en="Instead of moving parties one at a time, the Demographics tab moves whole voter blocs along seven axes — youth turnout, senior turnout, urban/rural skew, education, economic precarity, income and gender. Each party carries a sensitivity coefficient on each axis; pushing a slider shifts every party at once by an amount proportional to that coefficient, after which the field is renormalised to 100%."
                el="Αντί να μετακινεί κόμματα ένα τη φορά, η καρτέλα Δημογραφικά μετακινεί ολόκληρες ομάδες ψηφοφόρων σε επτά άξονες — προσέλευση νέων, προσέλευση ηλικιωμένων, κλίση αστικού/αγροτικού, εκπαίδευση, οικονομική επισφάλεια, εισόδημα και φύλο. Κάθε κόμμα έχει έναν συντελεστή ευαισθησίας σε κάθε άξονα· η μετακίνηση ενός ρυθμιστή μετατοπίζει κάθε κόμμα ταυτόχρονα κατά ένα ποσό ανάλογο με αυτόν τον συντελεστή, μετά το οποίο το πεδίο κανονικοποιείται εκ νέου στο 100%."
              />
            </p>
            <Mono>
            ...
            <p style={{ margin: "8px 0 0" }}>
              <T lang={lang}
                en={<>The effect is also <strong>geographic</strong>. Six of the seven axes map to real per-constituency measures — 65+ share, urbanisation, tertiary education, unemployment and GDP per capita (income) — standardised into population-weighted z-scores. On top of the uniform national swing, each party gets a local nudge proportional to how over- or under-represented that group is in the district — so a rise in senior turnout lifts senior-leaning parties more in older constituencies (Evrytania, Serres, Arcadia) than in young ones (the Dodecanese, East Attica). Because the z-scores are population-weighted they are nationally mean-zero, so this only redistributes the swing across the map; the national total is unchanged. <strong>Gender has no geographic component</strong> — no district-level field exists for it, so it stays a national-only shift.</>}
                el={<>Η επίδραση είναι επίσης <strong>γεωγραφική</strong>. Έξι από τους επτά άξονες αντιστοιχούν σε πραγματικά μεγέθη ανά περιφέρεια — ποσοστό 65+, αστικοποίηση, τριτοβάθμια εκπαίδευση, ανεργία και κατά κεφαλήν ΑΕΠ (εισόδημα) — τυποποιημένα σε z-scores σταθμισμένα με τον πληθυσμό. Πάνω στην ομοιόμορφη εθνική μετατόπιση, κάθε κόμμα παίρνει μια τοπική προσαρμογή ανάλογη με το πόσο υπερ- ή υπο-εκπροσωπείται αυτή η ομάδα στην περιφέρεια — οπότε μια αύξηση στην προσέλευση ηλικιωμένων ανεβάζει περισσότερο τα κόμματα που τους προσελκύουν σε πιο ηλικιωμένες περιφέρειες (Ευρυτανία, Σέρρες, Αρκαδία) απ' ότι σε νεανικές (Δωδεκάνησα, Ανατολική Αττική). Επειδή τα z-scores είναι σταθμισμένα με τον πληθυσμό, έχουν εθνικό μέσο όρο μηδέν, οπότε αυτό απλώς αναδιανέμει τη μετατόπιση στον χάρτη· το εθνικό σύνολο παραμένει αμετάβλητο. <strong>Το φύλο δεν έχει γεωγραφική συνιστώσα</strong> — δεν υπάρχει αντίστοιχο πεδίο ανά περιφέρεια, οπότε παραμένει αμιγώς εθνική μετατόπιση.</>}
              />
            </p>
            <p style={{ margin: "8px 0 0" }}>
              <T lang={lang}
                en={<>Two honesty notes on the geographic data: <strong>youth</strong> has no direct census field, so it is proxied as the inverse of the 65+ share pending real 18–34 district data; and <strong>unemployment</strong> is only available at NUTS-2 (13-region) resolution, so every constituency inside a region shares one figure rather than its own. Both the unemployment and income (GDP per capita) figures are also <strong>scenario-year-aware</strong> — a precarity slider pushed on the 2012 scenario redistributes votes using that era's crisis-level unemployment map, not today's, since Greek regional unemployment roughly tripled between 2009 and its 2013 peak before a slow recovery.</>}
                el={<>Δύο σημειώσεις ειλικρίνειας για τα γεωγραφικά δεδομένα: η <strong>νεολαία</strong> δεν έχει άμεσο απογραφικό πεδίο, οπότε προσεγγίζεται ως το αντίστροφο του ποσοστού 65+ εν αναμονή πραγματικών δεδομένων περιφέρειας 18–34· και η <strong>ανεργία</strong> είναι διαθέσιμη μόνο σε ανάλυση NUTS-2 (13 περιφέρειες), οπότε κάθε εκλογική περιφέρεια εντός μιας περιφέρειας μοιράζεται ένα κοινό νούμερο αντί για δικό της. Τόσο η ανεργία όσο και το εισόδημα (κατά κεφαλήν ΑΕΠ) είναι επίσης <strong>ευαίσθητα στο έτος του σεναρίου</strong> — ένας ρυθμιστής επισφάλειας στο σενάριο του 2012 αναδιανέμει ψήφους χρησιμοποιώντας τον χάρτη ανεργίας εκείνης της εποχής κρίσης, όχι τον σημερινό, καθώς η περιφερειακή ανεργία στην Ελλάδα περίπου τριπλασιάστηκε από το 2009 έως την κορύφωσή της το 2013 πριν από μια αργή ανάκαμψη.</>}
              />
            </p>
            <p style={{ margin: "8px 0 0" }}>
              <T lang={lang}
                en={<>The demographic sliders also skew <strong>who turns out</strong>, not just who they vote for: each district's effective electorate (the weight it carries in the §100.8 cross-constituency ranking) is multiplied by exp(κ · Σ slider/10 · z-score) with κ = 0.06, then rescaled so the national electorate total is unchanged. Mobilising seniors nationally therefore both shifts shares toward senior-leaning parties AND makes elderly constituencies weigh more in the marginal-seat fight — both effects of the same underlying mechanism.</>}
                el={<>Οι δημογραφικοί ρυθμιστές επηρεάζουν επίσης <strong>ποιος προσέρχεται</strong> να ψηφίσει, όχι μόνο ποιον ψηφίζει: το ενεργό εκλογικό σώμα κάθε περιφέρειας (το βάρος της στην κατάταξη §100.8) πολλαπλασιάζεται με exp(κ · Σ ρυθμιστής/10 · z-score) με κ = 0.06, και κατόπιν επανακλιμακώνεται ώστε το εθνικό σύνολο εκλογικού σώματος να παραμένει αμετάβλητο. Η κινητοποίηση ηλικιωμένων σε εθνικό επίπεδο συνεπώς μετατοπίζει τα ποσοστά προς κόμματα που τους προσελκύουν ΚΑΙ κάνει τις γηραιότερες περιφέρειες να βαραίνουν περισσότερο στη μάχη των οριακών εδρών — δύο όψεις του ίδιου μηχανισμού.</>}
              />
            </p>
            <p style={{ margin: "8px 0 0", color: "var(--text-muted)", fontSize: 13 }}>
              <T lang={lang}
                en="Sensitivities start as hand-set priors, then a subset are blended 50/50 with coefficients from a weighted least-squares regression of each party's 2019→2023 district-level residual swing (the part not explained by the uniform national swing and stronghold damping) against the district z-scores — blended only where the estimate clears a |t| > 2 significance bar, otherwise the hand prior is kept. Fit quality varies a lot by party (R² from ≈0.02 for a geographically diffuse party up to ≈0.50 for the most geographically sorted one); this is a real, disclosed limitation of a 59-district, single-election-pair calibration, not a hidden precision claim."
                el="Οι ευαισθησίες ξεκινούν ως χειροκίνητες εκτιμήσεις, και ένα υποσύνολο συνδυάζεται 50/50 με συντελεστές από μια σταθμισμένη παλινδρόμηση ελαχίστων τετραγώνων της υπολειπόμενης μετατόπισης κάθε κόμματος ανά περιφέρεια από το 2019 στο 2023 (το μέρος που δεν εξηγείται από την ομοιόμορφη εθνική μετατόπιση και την απόσβεση προπυργίου) ως προς τα z-scores περιφέρειας — ο συνδυασμός γίνεται μόνο όπου η εκτίμηση ξεπερνά όριο σημαντικότητας |t| > 2, αλλιώς διατηρείται η χειροκίνητη τιμή. Η ποιότητα προσαρμογής διαφέρει πολύ ανά κόμμα (R² από ≈0.02 για ένα γεωγραφικά διάχυτο κόμμα έως ≈0.50 για το πιο γεωγραφικά συγκεντρωμένο)· αυτός είναι ένας πραγματικός, δηλωμένος περιορισμός μιας βαθμονόμησης 59 περιφερειών με ένα μόνο ζεύγος εκλογών, όχι κρυφή αξίωση ακρίβειας."
              />
            </p>
          </Section>
          ...
          <Section step="C2" title={t("District-level logit swing")}>
            ...
            <Mono>
              ...
              damping = max(0.4, 1 − ((localBase − 5) ÷ 90) × 0.6)<br/>
              localShare(p, d) = logit⁻¹( logit(localBase) + swing(p) × damping + geoNudge(p, d) )
            </Mono>
            <p style={{ margin: 0 }}>
              <T lang={lang}
                en={<>For a locally dominant party the damping floors at 0.4×, so even a large national collapse can still cut through a stronghold rather than leaving it untouchable; for a micro-party (base &lt; 5%) it intentionally exceeds 1.0×, so a national surge can realistically push it over 3% in its strongholds. The demographic sliders' geographic nudge (Part C1) is added in the same logit step, clamped to ±0.6 logits after summing all seven axes. Each district is then renormalised to 100%.</>}
                el={<>Για ένα τοπικά κυρίαρχο κόμμα η απόσβεση φτάνει στο κατώφλι 0.4×, ώστε ακόμη και μια μεγάλη εθνική κατάρρευση να μπορεί να «κόψει» ένα προπύργιο αντί να το αφήνει άτρωτο· για ένα μικρό κόμμα (βάση &lt; 5%) ξεπερνά σκόπιμα το 1.0×, ώστε μια εθνική άνοδος να μπορεί ρεαλιστικά να το πάει πάνω από 3% στα προπύργιά του. Η γεωγραφική προσαρμογή των δημογραφικών ρυθμιστών (Μέρος C1) προστίθεται στο ίδιο βήμα logit, με όριο ±0.6 logits μετά την άθροιση και των επτά αξόνων. Κάθε περιφέρεια κανονικοποιείται εκ νέου στο 100%.</>}
              />
            </p>
            <p style={{ margin: "8px 0 0", color: "var(--text-muted)", fontSize: 13 }}>
              <T lang={lang}
          ...
          <section style={{ padding: 16, background: "rgba(239, 68, 68, 0.05)", border: "1px solid rgba(239, 68, 68, 0.2)", borderRadius: 8 }}>
            ...
            <ul style={{ margin: 0, paddingLeft: 20, display: "flex", flexDirection: "column", gap: 6, fontSize: 13, color: "var(--text-muted)" }}>
              <T lang={lang}
                en={<>
                  <li>The demographic sensitivities and district swing multipliers are <strong>modelling assumptions</strong>, hand-set priors partly blended with a WLS regression on 2019→2023 patterns (Part C1), not coefficients estimated from individual-level survey microdata. Brand-new parties without electoral history are approximated through the lineage map and should be read with caution.</li>
                  <li><strong>Backtest</strong> (2019→2023, zero demographic sliders — national swing + damping only, evaluated against the real 2023 result): district-level vote-share error averages ≈1.7 percentage points per party per district, with the largest misses in constituencies with large linguistic-minority electorates (e.g. Rhodope) that a purely demographic model doesn't capture; no constituency seat was misallocated relative to the official outcome in this backtest.</li>
                ...
                el={<>
                  <li>Οι δημογραφικές ευαισθησίες και οι πολλαπλασιαστές μετατόπισης περιφέρειας είναι <strong>υποθέσεις μοντελοποίησης</strong>, χειροκίνητες εκτιμήσεις εν μέρει συνδυασμένες με παλινδρόμηση WLS στα πρότυπα 2019→2023 (Μέρος C1), όχι συντελεστές εκτιμημένοι από μικροδεδομένα ερευνών σε ατομικό επίπεδο. Ολοκαίνουργια κόμματα χωρίς εκλογικό ιστορικό προσεγγίζονται μέσω του χάρτη καταγωγής και πρέπει να διαβάζονται με προσοχή.</li>
                  <li><strong>Οπισθοδοκιμή</strong> (2019→2023, μηδενικοί δημογραφικοί ρυθμιστές — μόνο εθνική μετατόπιση + απόσβεση, αξιολογημένη έναντι του πραγματικού αποτελέσματος 2023): το σφάλμα ποσοστού ψήφων ανά περιφέρεια είναι κατά μέσο όρο ≈1.7 ποσοστιαίες μονάδες ανά κόμμα ανά περιφέρεια, με τα μεγαλύτερα σφάλματα σε περιφέρειες με μεγάλο γλωσσικό-μειονοτικό εκλογικό σώμα (π.χ. Ροδόπη) που ένα καθαρά δημογραφικό μοντέλο δεν συλλαμβάνει· καμία έδρα περιφέρειας δεν κατανεμήθηκε λανθασμένα σε σχέση με το επίσημο αποτέλεσμα σε αυτή την οπισθοδοκιμή.</li>
